...
const useAuthInternal = () => {
  ...
  useEffect(() => {
    if (currentUserData?.user && currentUserData.isAuthenticated) {
      // Update context with fresh user data
      login(currentUserData.user);
    } else if (isError && queryError?.message === "Unauthorized") {
  ...
  }, [currentUserData, isError, queryError, login, logout]);
...
export const useSignup = () => {
  ...
  return { mutate: signupMutation.mutate, mutateAsync: signupMutation.mutateAsync, isPending: signupMutation.isPending };
};
...
export const useLogin = () => {
  ...
  return { mutate: loginMutation.mutate, mutateAsync: loginMutation.mutateAsync, isPending: loginMutation.isPending };
};
...
export const useLogout = () => {
  ...
  return { mutate: logoutMutation.mutate, mutateAsync: logoutMutation.mutateAsync, isPending: logoutMutation.isPending };
};

export const useCurrentUser = () => {
  const { user, isLoading, isAuthenticated, error, refetchCurrentUser, login, logout } = useAuthInternal();
  return {
    user,
    isLoading,
    isAuthenticated,
